    ...
    return (
        <div
          ...
          >
            {
              brands.map((brand)=>{
                return <Option value={brand.brand_id}>{brand.brand_name_tm}</Option>
              })
            }
          </Select>
          <Select
          ...
          <Select
           className='suruji-yagdayy--input'
            // style={{ width: 200 }}
            placeholder="SubKategoriýa Saýla"
            onChange={(value)=>setSubcategory_id(value)}
          >
            {
             subkategoriya && subkategoriya.map((kategor)=>{
                return <Option value={kategor.subcategory_id}>{kategor.subcategory_name_tm}</Option>
              })
            }
          </Select>
           ...
           <Input value={product_discount} onChange={(e)=>{setProduct_discount(e.target.value)}} addonBefore='Prasent skitga'  className='suruji-yagdayy--input' />
                ...
                <Button style={{width:"40%"}} onClick={CreateProduct} icon={<PlusCircleFilled/>} shape='round' type='primary' className='suruji-yagdayy--button'>Hasaba al</Button>
                <Button style={{width:"40%"}} onClick={props.onClick} shape='round' danger type='primary' className='suruji-yagdayy--button'>Cancel</Button>
                </div>
            </form>
        </div>
    );
